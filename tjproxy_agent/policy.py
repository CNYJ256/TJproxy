...
def merge_legacy_powershell_commands(
    config: PolicyConfig, legacy_commands: tuple[Any, ...]
) -> PolicyConfig:
    ...
    for legacy in legacy_commands:
        ...
        if folded in commands:
            continue
        commands[folded] = CommandRule(
            name=name,
            allow_subcommands=tuple(getattr(legacy, "allowed_subcommands", ())),
            deny_args=tuple(getattr(legacy, "denied_args", ())),
        )
    return replace(config, commands=commands)
...
class PolicyEngine:
    ...
    def _review_powershell(
        ...
        for stage in pipeline:
            ...
            if rule is None:
                if _is_destructive_command(command, args):
                    return ReviewResult(
                        PolicyDecision.DENY,
                        reason=f"destructive command is denied: {command}",
                        error_code="POLICY_DENIED",
                        call_hash=_call_hash(call, context),
                    )
                return ReviewResult(
                    PolicyDecision.APPROVAL_REQUIRED,
                    reason=f"unknown command requires approval: {command}",
                    risk=Risk.LONG_RUNNING,
                    summary=f"{command} {' '.join(args)}".strip(),
                    details={"command": command, "args": args, "source": "unknown"},
                    call_hash=_call_hash(call, context),
                )
    ...
    def _review_command_rule(
        ...
        if command.casefold() == "npm" and first == "run" and len(args) >= 2:
            script_name = args[1]
            script = _read_npm_script(context.workspace, script_name)
            if script and _looks_like_service_script(script):
                return ReviewResult(
                    PolicyDecision.APPROVAL_REQUIRED,
                    reason=(
                        "package.json script appears to start a service: "
                        f"npm run {script_name}"
                    ),
                    risk=Risk.SERVICE_START,
                    summary=f"npm run {script_name}",
                    details={"script": script, "source": "package.json"},
                    call_hash=call_hash,
                )
...
def _is_destructive_command(command: str, args: list[str]) -> bool:
    folded = command.casefold()
    if folded in {"rm", "del", "erase", "rmdir", "rd", "remove-item"}:
        return True
    destructive_args = {"-r", "-rf", "-recurse", "-force", "/s", "/q"}
    return any(arg.casefold() in destructive_args for arg in args) and folded in {
        "remove-item",
        "rm",
        "del",
        "rmdir",
...
def _read_npm_script(workspace: Path, script_name: str) -> str | None:
...
def _looks_like_service_script(script: str) -> bool:
    lowered = script.casefold()
    return any(
        token in lowered
        for token in (" dev", "serve", "--host", "listen", "server")
    )
